...
function Notes(){
  ...
  const provider_id = useSelector(store=>store.authReducer.user.provider_id)
  ...
  const handleSubmit=()=>{
    axios.post(`api/provider/assignments/${provider_id}`,{client_id: 11,providerList})
    .then((res=>{
  ...
  const handleEdit=(i, newText)=>{
    setProviderItem(providerList[i])
    console.log('provider at index', providerList[i])
    setButtonLabel('Save Changes')
    debugger;
    const editList = providerList.map((item, index)=>  index === i ? newText : item)
        ...
        <div className="p_CN_tiles_container">
          ...
          <div className="p_CN_intro_text p_CN_tile">
            ...
            <div className='p_CN_Assignment_adder provider_btnContainer'>
              <button onClick={handleOnClick}><h3>{buttonLabel}</h3></button>
            </div>
            ...
            <div className='p_CN_Assignment_display'>
             ...
             {return <table className = 'assignments'>
               <tr>
                 <>{e}</>
                 <td><button  onClick={ ()=>handleEdit(index) }className='Edit'>Edit</button></td>
                 <td><button onClick={()=> handleDelete(index)} className='delete'>Delete</button></td>
             </tr>
             </table>
             })}
            </div>
